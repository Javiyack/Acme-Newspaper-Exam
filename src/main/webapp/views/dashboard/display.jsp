...
<div class="dashboard">
<h5><spring:message code="dashboard.level"/> Exam</h5>
	<br>
	<!-- A4	The ratio of subscribers per private newspaper versus the total number of customers. -->
	<h3>
		EXAMEN QUERY 2
	</h3>
	<display:table class="displaytag" name="examQuery2" id="row">
		<spring:message code="master.page.administrator" var="title" />
		<display:column title="${title }">
			<jstl:out value="${row[0] }" />
		</display:column>

		<spring:message code="dashboard.maximum" var="deviationB4Title" />
		<display:column title="${deviationB4Title }">
			<jstl:out value="${row[1] }" />
		</display:column>
	</display:table>
